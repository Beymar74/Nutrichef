...
<head>
    ...
    <title>@yield('titulo', 'Admin') - NutriChef</title>
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-30 w-64 bg-ambar-800 text-white transition-transform duration-300 ease-in-out -translate-x-full lg:translate-x-0 lg:static flex flex-col shadow-2xl lg:shadow-none border-r border-ambar-900">
            ...
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
                ...
                <a href="{{ route('admin.dashboard') }}" 
                   ...
                   {{ request()->routeIs('admin.dashboard') ? 'bg-calabaza-600 text-white shadow-md shadow-calabaza-900/20 transform hover:-translate-y-0.5' : 'text-ambar-50 hover:bg-ambar-900/50 hover:text-white' }}">
                    <i data-lucide="layout-dashboard" class="w-5 h-5 {{ request()->routeIs('admin.dashboard') ? '' : 'group-hover:text-calabaza-500 transition-colors' }}"></i>
                    <span>Dashboard</span>
                </a>
                ...
                <a href="{{ route('admin.usuarios.index') }}" 
                   ...
                   {{ request()->routeIs('admin.usuarios.*') ? 'bg-calabaza-600 text-white shadow-md shadow-calabaza-900/20 transform hover:-translate-y-0.5' : 'text-ambar-50 hover:bg-ambar-900/50 hover:text-white' }}">
                    <i data-lucide="users" class="w-5 h-5 {{ request()->routeIs('admin.usuarios.*') ? '' : 'group-hover:text-calabaza-500 transition-colors' }}"></i>
                    <span>Usuarios</span>
                </a>
                ...
                <a href="{{ route('admin.recetas.index') }}" 
                   ...
                   {{ request()->routeIs('admin.recetas.*') ? 'bg-calabaza-600 text-white shadow-md shadow-calabaza-900/20 transform hover:-translate-y-0.5' : 'text-ambar-50 hover:bg-ambar-900/50 hover:text-white' }}">
                    <i data-lucide="utensils-crossed" class="w-5 h-5 {{ request()->routeIs('admin.recetas.*') ? '' : 'group-hover:text-calabaza-500 transition-colors' }}"></i>
            ...
            <div class="p-4 border-t border-ambar-900 bg-ambar-950/30">
                <div class="flex items-center gap-3 mb-3 px-2">
                    ...
                    <div class="overflow-hidden">
                        ...
                        <p class="text-xs text-ambar-200 truncate w-32">{{ Auth::user()->email ?? '' }}</p>
                    </div>
                </div>
            ...
            <header class="bg-white border-b border-slate-200 shadow-sm h-16 flex items-center justify-between px-6 lg:px-8 z-10">
                <div class="flex items-center gap-4">
                    ...
                    <h2 class="text-lg font-bold text-slate-800 hidden sm:block flex items-center gap-2">
                        ...
                        @yield('titulo_pagina', 'Panel Administrativo')
                    </h2>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <!-- Info Usuario (Navbar) -->
                    <span class="text-sm text-slate-600 font-medium hidden sm:block">Hola, {{ Auth::user()->name ?? 'Admin' }}</span>
                </div>
            </header>
